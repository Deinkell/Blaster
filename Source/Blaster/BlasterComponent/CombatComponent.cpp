...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if(Controller)
	{
		...
		{			
			...
			HUDPackage.CrosshairsSpread = 0.5f + CrosshairVelocityFactor + CrosshairInAirFactor - 
...
void UCombatComponent::FireProjectileWeapon()
{
	if(EquippedWeapon && Character)
	{
		HitTarget = EquippedWeapon->bUserScatter ? EquippedWeapon->TraceEndWithScatter(HitTarget) : HitTarget;
...
bool UCombatComponent::ServerFire_Validate(const FVector_NetQuantize& TraceHitTarget, float FireDelay)
{
	if(EquippedWeapon)
	{
		bool NearlyEqual = FMath::IsNearlyEqual(EquippedWeapon->FireDelay, FireDelay, 0.001f);
		return NearlyEqual;
	}
...
void UCombatComponent::TraceUnderCrosshairs(FHitResult& TraceResult)
{
	...
	if(TraceResult.GetActor() && TraceResult.GetActor()->Implements<UInteractWithCrosshairsInterface>())
	{		
...
void UCombatComponent::UpdateAmmoValues()
{
	...
	int32 ReloadAmount = AmountReload();
	if(CarriedAmmoMap.Contains(EquippedWeapon->GetWeaponType()))
	{
...
int32 UCombatComponent::AmountReload()
{
